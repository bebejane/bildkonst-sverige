...
import React, { useState, forwardRef, useCallback, CSSProperties, useRef, useEffect, version } from 'react';
import { useInView } from 'react-intersection-observer';
import { encode } from 'universal-base64';

const isSsr = typeof window === 'undefined';

const isIntersectionObserverAvailable = isSsr ? false : !!(window as any).IntersectionObserver;

function fetchPriorityProp(fetchPriority?: string): Record<string, string | undefined> {
...
const buildSrcSet = (src: string | null | undefined, width: number | undefined, candidateMultipliers: number[]) => {
	if (!(src && width)) {
		return undefined;
	}
...
const Image = forwardRef<HTMLDivElement, ImagePropTypes>(
	(
		{
			className,
			fadeInDuration = 500,
		...
		const webpSource = data.webpSrcSet && (
			<source srcSet={data.webpSrcSet} sizes={sizes ?? data.sizes ?? undefined} type='image/webp' />
		);
