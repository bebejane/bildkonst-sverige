...
export default async function ReadMore({ url, path, className, link, external = false }: ReadMoreProps) {
	if (link && link.__typename === 'InternalLinkRecord') path = (await recordToRoute(link.link))?.[0];
	else if (link && link.__typename === 'ExternalLinkRecord') url = link.url;

	return (
		<>
			{url ? (
				<a href={url} className={cn(s.read, className, 'date')} target='_blank' rel='noreferrer'>
					Öppna <span className={s.external}>»</span>
				</a>
			) : path ? (
				<Link href={path} className={cn(s.read, className, 'date')}>
					Läs mer »
				</Link>
			) : (
