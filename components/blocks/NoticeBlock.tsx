...
export default async function NoticeBlock({ data: { id, headline, image, link, text, category, date } }: Props) {
	const href = link?.__typename === 'InternalLinkRecord' ? (await recordToRoute(link.link))?.[0] : link?.url;
	...
	const content = (
		<>
			...
			{category && date && (
				<span className='date'>
					{category} • {format(new Date(date), 'yyyy-MM-dd')}
				</span>
			)}
			<h4>{headline}</h4>
			...
			<p className='small'>
				{text && text} {text && ' '}
				{link && <ReadMore className='date' external={link?.__typename === 'ExternalLinkRecord'} />}
			</p>
		</>
	);
