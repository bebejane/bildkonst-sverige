...
const MemberForm = z.object({
	...
	level: z
		.string()
		.min(2, { message: 'Medlemsnivå ID är ogiltig' })
		.refine((s) => s !== 'false', 'Medlemsnivå är obligatorisk'),
});
...
export default async function createMember(
	prevState: any,
	formData: FormData
): Promise<{ data?: any; error?: string; invalid?: any[] }> {
	try {
		...
		if (!mail) throw new Error('No mail template texts found');
